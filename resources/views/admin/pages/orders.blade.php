<div>
    <div class="row g-4 mb-4">
        <div class="row g-3">
            ...
            <!-- 1) Total Orders -->
            <livewire:admin.components.dashboard-widget :title="'Total Orders'" :value="$this->orders->total()" :color="'primary'"
                :icon="'vendors/@coreui/icons/svg/free.svg#cil-cart'" :chartId="'card-total-orders'" :dropdownItems="['All']" />

            <!-- 2) Pending Orders -->
            <livewire:admin.components.dashboard-widget :title="'Pending Orders'" :value="$totalPending" :subtext="round(($totalPending / $this->orders->total()) * 100, 1) . '%'"
                :color="'warning'" :icon="'vendors/@coreui/icons/svg/free.svg#cil-clock'" :chartId="'card-pending-orders'" :dropdownItems="['Pending']" />

            <!-- 3) Completed Orders -->
            <livewire:admin.components.dashboard-widget :title="'Completed Orders'" :value="$totalCompleted" :subtext="round(($totalCompleted / $this->orders->total()) * 100, 1) . '%'"
                :color="'success'" :icon="'vendors/@coreui/icons/svg/free.svg#cil-check-circle'" :chartId="'card-completed-orders'" :dropdownItems="['Completed']" />

            <!-- 4) Canceled Orders -->
            <livewire:admin.components.dashboard-widget :title="'Canceled Orders'" :value="$totalCanceled" :subtext="round(($totalCanceled / $this->orders->total()) * 100, 1) . '%'"
                :color="'danger'" :icon="'vendors/@coreui/icons/svg/free.svg#cil-x-circle'" :chartId="'card-canceled-orders'" :dropdownItems="['Canceled']" />
    ...
    <div class="p-4 bg-white rounded shadow">
                                    ...
                                    <tr class="align-middle">
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                <button class="btn btn-sm btn-info btn-action" title="Details Order"
                                                    ...
                                                    <svg class="nav-icon text-white"
                                                        ...
                                                        <use
                                                            xlink:href="vendors/@coreui/icons/svg/free.svg#cil-airplay">
                                                        </use>
                                                    </svg>
                                                </button>
        ...
        @if ($showDetailsModal && $detailsOrderId)
            <div class="modal fade show d-block" tabindex="-1" role="dialog" aria-labelledby="orderModalLabel"
                wire:click.self="closeDetailsModal" style="background-color: rgba(0,0,0,0.5);">
