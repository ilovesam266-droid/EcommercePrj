<div>
    <div>
        <div class="row g-4 mb-4">
            <div class="row g-3">
                <!-- 1) Total Addresses -->
                <livewire:admin.components.dashboard-widget :title="'Addresses'" :value="$this->addresses->total()" :color="'primary'"
                    :icon="'vendors/@coreui/icons/svg/free.svg#cil-address-book'" :chartId="'card-total-spark'" :dropdownItems="['All']" />

                <!-- 2) Default Addresses -->
                <livewire:admin.components.dashboard-widget :title="'Default Addresses'" :value="$totalDefault" :subtext="round(($totalDefault / $this->addresses->total()) * 100, 1) . '%'"
                    :color="'success'" :icon="'vendors/@coreui/icons/svg/free.svg#cil-arrow-top'" :chartId="'card-default-pie'" :dropdownItems="['Default']" />

                <!-- 3) Addresses Needing Attention (Missing Fields) -->
                <livewire:admin.components.dashboard-widget :title="'Needs Attention (Missing Fields)'" :value="$missingField" :color="'warning'"
                    :icon="'vendors/@coreui/icons/svg/free.svg#cil-warning'" :chartId="'card-invalid-bar'" :dropdownItems="['Missing Information']" />

                <!-- 4) Top Provinces -->
                <livewire:admin.components.dashboard-widget :title="'Distribution by Province'" :value="$topProvinces->sum('total')" :color="'info'"
                            ...
                            <tbody>
                                @foreach ($this->addresses as $address)
                                    <tr class="align-middle">
                                        ...
                                        <td>
                                            <div style="max-width: 300px;">
                                                <div class="text-truncate fw-medium" style="max-width: 420px;">
                                                    {{ Str::limit($address->detailed_address, 43) }}
                                        ...
                                        <td>
                                            <div class="text-truncate fw-medium">{{ $address->recipient_name }}
                                            </div>
                                            <div class="small text-body-secondary text-nowrap">
                                        ...
                                        <td>
                                            <div class="text-truncate fw-medium">{{ $address->user->first_name }}
                                                {{ $address->user->last_name }}
                                                @if ($address->user->status->value ?? null)
                                                    <span
                                                        class="badge ms-1 {{ $address->user->status->value === 'active' ? 'bg-success' : 'bg-secondary' }}">
                                                        {{ ucfirst($address->user->status->value) }}
                                                    </span>
                                                @endif
                                            </div>
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                <button class="btn btn-sm btn-warning btn-action" data-bs-toggle="tooltip"
                                                    title="Edit" wire:click="openEditModal({{ $address->id }})">
...
<script>
    ...
    const ctx = document.getElementById('card-top-province').getContext('2d');

    const labels = @json($topProvinces->pluck('province'));
    const data = @json($topProvinces->pluck('total'));
